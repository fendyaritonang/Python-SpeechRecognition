...
8. pip3 install pyttsx3
8. sudo apt-get install flac
9. Attach your microphone and execute this the following command to check if you have audio input device
   arecord -l
10. If it's listed, execute the following:
    python3 checkMicrophone.py
11. It should list all the available audio input. Yours should be hw:1, 0
12. Execute the following command to adjust audio output and input volume 
    alsamixer
13. The first one should be your audio output device. Pls adjust the volumn by clicking up/down arrow
14. To adjust audio input device, press F6 and select your microphone. Then adjust the volume using up/down arrow
15. To exit from mixer, press Esc
16. Test the recording using the following command. Record duration is 5 seconds and it will record to test.wav
    arecord -f dat -r 60000 -D plughw:1,0 -d 5 test.wav
17. Play the recording using the following command
    aplay test.wav
18. If everything is good, now we're ready for the speech SpeechRecognition.
